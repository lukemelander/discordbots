...
class Screeners(commands.Cog, name= 'Screener'):
    ...
    async def screenhelp2(self, ctx):
    ...
    async def screenhelp3(self, ctx):
    ...
    async def screenhelp4(self, ctx):
    ...
    async def on_message(self, message):
        ...
        if bool(m):
            ...
            ticker = re.compile(r"\s\S*\s([a-zA-Z]*)").split(msg)[1].lstrip()
        ...
        if bool(m):
            ...
            try:
                ...
                response['Insider Trading'] = response['Insider Trading'].astype(str).replace(r'(?<=\s.{1}).*',"",regex=True)
...
async def setup(bot):
    await bot.add_cog(Screeners(bot))
