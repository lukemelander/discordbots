...
class Speech(commands.Cog, name='Speech'):
    ...
    async def on_message(self, message):
        ...
        m = re.search('W[H]*EN OTC', message.content.upper())
        boolean = bool(m)
        if boolean:
            response = random.choice(whenotc)
            await message.channel.send(response)

        m = re.search('W[H]*EN MOON', message.content.upper())
        boolean = bool(m)
        if boolean:
            response = random.choice(whenotc)
            await message.channel.send(response)
    ...
    The Zeroth Law: A robot may not harm humanity, or, by inaction, allow humanity to come to harm.'''
        ...
        m = re.search('COMBOT IS BACK', message.content.upper())
        boolean = bool(m)
        if boolean:
            response = "News of my demise was greatly exaggerated"
            await message.channel.send(response)

        m = re.search('MOIST ?ISSUES', message.content.upper())
        boolean = bool(m)
        if boolean:
            await message.channel.send(file=discord.File('images/moistissues.png'))
